...
server <- function(input, output) {
  ...
  realisation <- reactive({
    ...
    ##### General setup
    compt <- c("S", "E", "I", "R") # SEIR compartment
    ...
    no_sub_compt <- c("S" = 1, "E" = 5, "I" = 5, "R" = 1) # store number of sub-compartments
    sub_names <- list("S" = NULL, "E" = NULL, "I" = NULL, "R" = NULL)
    SYS_compt <- NULL
    for (c in compt) {
    ...
    colnames(SYS_N) <- c(sub_names[["S"]], sub_names[["E"]], sub_names[["I"]], sub_names[["R"]])
    ...
    for (r in 1:real) {
      for (i in 1:N_phases) {
        if (i == 1) { # for first phase of the epimedic
          res <- seir_model_erlang(t_phase, t, dt, N, mean_holding_times, beta, SYS_N, sub_names)
        } else {
  ...
  output$plot <- renderPlot({
    ...
    plt2 <- plt1 +
      scale_y_log10(limits = c(1E-2, NA), breaks = scales::trans_breaks("log10", function(x) 10^x), labels = scales::trans_format("log10", math_format(10^.x))) +
  ...
  output$Report <- renderUI({
    fit <- survival::survfit(Surv(d()$x) ~ 1)
